...
router.get('/allCats', function(req, res) {
	...
	connections.getConnection(res, function(cnn) {
		cnn.query(' SELECT id_cat, categories from Categories ',
		function(err, result){
			if(err){
...
router.post('/addNewCat', function(req, res) {
   ...
   var body = req.body;
	...
	if(vld.check(admin, Tags.noPermission) && vld.hasFields(body, ['newCategory'])) {
		connections.getConnection(res, function(cnn) {
			cnn.query(' INSERT INTO catergoies (categories) values (?) ', body.newCategory,
			function(err, result){
				if(err) {
...
// No need Authorization to do this
router.get('/getManufacturesBasedOnDeviceType', function(req, res) {
	console.log("get manufactures for a sepecific category");
	var vld = req.validator;
	var body = req.body;

	if(vld.hasFields(body, ['categoryId'])) {
		connections.getConnection(res, function(cnn) {
			cnn.query(' SELECT m.id_man, m.manufacturer FROM manufacturers m, categoriesmanufacturers c' +
						 'WHERE c.man_id = m.id_man AND c.cat_id = ?', body.categoryId,
				function(err, result) {
					if(err) {
...
router.get('/getModelsBasedOnCategoryAndManufacture', function(req, res) {
	console.log("get models");
	var vld = req.validator;
	var body = req.body;
	if(vld.hasFields(body,['categoryId', 'manufactureId'])) {
		connections.getConnection(res, function(cnn) {
			cnn.query(' SELECT id_catMan AS id_model, model FROM categoriesmanufacturers' +
						 ' WHERE cat_id = ? AND man_id = ? ', body.categoryId, body.manufactureId,
				function(err, result) {
					if(err) {
...
// Require categoryId
// No need Authorization for this
router.get('/getIssuesBasedOnCategory', function(req, res) {
		console.log("get issues based on category");
		var vld = req.validator;
		var body = req.body;
		if (vld.hasFields(body, ['categoryId'])) {
			connections.getConnection(res, function(cnn) {
				cnn.query('SELECT id_catIss, issues from categoriesIssues WHERE cat_id = ?', body.categoryId,
				function(err, result) {
					if (err) {
